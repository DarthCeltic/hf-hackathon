...
int main(uintptr_t arg_area)
{
     ...
     * iterations -- the expensive, embarrassingly-parallel-per-anchor part,
     * same order of magnitude as the 241k-call sigmoid loop PR#64 already
     * fixed elsewhere) to find best-class+score per anchor and threshold-
     * filter. Cannot compact into a survivors list here (that needs a
     * shared n_cands++ counter -- no atomics exist anywhere in this file),
     * so instead every hart writes its own disjoint anchor range into a
     * FIXED dense[3024] array (alive=0/1 flag, no compaction, no shared
     * state, no race). struct DCand is padded to exactly 32 bytes so 2
     * structs = 1 64-byte cache line; the 3024-anchor range is quantized
     * to blocks of 2 anchors BEFORE dividing across harts (num_blocks=1512)
     * -- the same block-quantization principle the DFL decode fix uses for
     * flat float arrays, generalized here to struct-pairs -- so every
     * hart's dense[] write span starts and ends on a whole cache line. */
    ...
    if (is_h0) {
         ...
         * O(n^2) NMS below needs. This keeps NMS's cost structure exactly
         * as before (n ~100 survivors, not 3024) -- only the expensive
         * scan above was parallelized, not the sequential NMS/top-K. */
