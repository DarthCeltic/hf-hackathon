...
Current repo note: this journal records the historical M-series optimization
path, including the original M30 `web_car` bring-up. The current CI gate still
uses the M30 kernel, but validates it across the five-image static suite
configured in `.github/ci/benchmark_config.json`.
